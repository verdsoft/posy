...
export default function PurchaseReturnsReport() {
  ...
  const handleExportPDF = () => {
    ...
    const tableData = filteredReturns.map(returnItem => [
      ...
      returnItem.status,
      `$${returnItem.total || "0.00"}`,
      `$${returnItem.paid || "0.00"}`,
      `$${returnItem.due || "0.00"}`,
  ...
  const handleExportExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredReturns.map(returnItem => ({
        ...
        Status: returnItem.status,
        Total: `$${returnItem.total || "0.00"}`,
        Paid: `$${returnItem.paid || "0.00"}`,
        Due: `$${returnItem.due || "0.00"}`,
                ...
                {loading ? (
                  ...
                  filteredReturns.map((returnItem) => (
                    <tr key={returnItem.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">${returnItem.total || "0.00"}</td>
                      <td className="p-3">${returnItem.paid || "0.00"}</td>
                      <td className="p-3">${returnItem.due || "0.00"}</td>
                      <td className="p-3">
                        <span className={`px-2 py-1 rounded text-xs ${
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="grid grid-cols-2 gap-4 p-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm">${selectedReturn.subtotal || "0.00"}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Tax Amount</label>
                  <p className="mt-1 text-sm">${selectedReturn.tax_amount || "0.00"}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Discount</label>
                  <p className="mt-1 text-sm">${selectedReturn.discount || "0.00"}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Shipping</label>
                  <p className="mt-1 text-sm">${selectedReturn.shipping || "0.00"}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Total</label>
                  <p className="mt-1 text-sm">${selectedReturn.total || "0.00"}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Paid</label>
                  <p className="mt-1 text-sm">${selectedReturn.paid || "0.00"}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Due</label>
                  <p className="mt-1 text-sm">${selectedReturn.due || "0.00"}</p>
                </div>
