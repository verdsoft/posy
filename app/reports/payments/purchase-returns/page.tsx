...
import { useGetPurchaseReturnPaymentsQuery } from "@/lib/slices/paymentsApi"
...
export default function PurchaseReturnsPayments() {
  ...
  const { data, isLoading, isFetching } = useGetPurchaseReturnPaymentsQuery({
  ...
  const handleExportPDF = () => {
    const doc = new jsPDF()
    doc.text('Purchase Return Payments Report', 14, 16)
    
    const tableData = payments.map(payment => [
      new Date(payment.date).toLocaleDateString(),
      payment.reference,
      payment.purchase_return_reference,
    ...
    autoTable(doc, {
      head: [['Date', 'Reference', 'Purchase Return', 'Supplier', 'Payment Method', 'Amount']],
      body: tableData,
      startY: 20,
      styles: { fontSize: 8 },
    ...
    doc.save('purchase-return-payments.pdf')
  }
  ...
  const handleExportExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      payments.map(payment => ({
        ...
        'Purchase Return': payment.purchase_return_reference,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Purchase Return Payments")
    XLSX.writeFile(workbook, "purchase-return-payments.xlsx")
  }
                ...
                {(isLoading || isFetching) ? (
                  ...
                  payments.map((payment) => (
                    <tr key={payment.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-3">{payment.purchase_return_reference}</td>
          ...
          <Dialog open={showViewModal} onOpenChange={setShowViewModal}>
            <DialogContent className="max-w-2xl">
              ...
              <div className="grid grid-cols-2 gap-4 p-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-500">Purchase Return Reference</label>
                  <p className="mt-1 text-sm">{selectedPayment.purchase_return_reference}</p>
                </div>
                <div>
